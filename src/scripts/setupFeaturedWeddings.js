...
// Updated to use ES Modules for better future compatibility
...
async function updateGalleryImages(galleryId, galleryType) {
  ...
  for (let j = 0; j < Math.min(existingImages.length, imageUrls.length); j++) {
    ...
    const imageRef = doc(db, 'galleries', galleryId, 'images', imageDoc.id);
...
// Run the script with top-level await (ES Modules feature)
try {
  await setupFeaturedWeddings();
