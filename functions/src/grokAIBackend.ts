...
interface Package {
  ...
  name: string;
  price: string;
  features: (string | { text: string; icon?: string })[]; // Simplified for backend
  usp?: string;
  perfectFor?: string[];
}

export const getGrokPackageSuggestion = functions.https.onRequest(async (req, res) => {
...
`;
  ...
  packages.forEach((pkg: Package, index: number) => {
    prompt += `
...
`;
  ...
  try {
    const axiosResponse = await axios.post(
      GROK_API_URL,
      {
        model: "grok-1.5-flash", // Or any other suitable Grok model
        messages: [
