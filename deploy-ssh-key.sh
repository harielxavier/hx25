#!/bin/bash

# Hariel Xavier Photography - SSH Key Deployment Script
# Deploy to harielxavier.com using SSH private key

echo "🚀 Starting SSH key deployment to harielxavier.com..."

# Configuration
SSH_HOST="harielxavier.com"
SSH_USER="missiongeek"
REMOTE_PATH="/public_html"
BUILD_DIR="./dist"
KEY_FILE="./ssh_key"

# Check if build directory exists
if [ ! -d "$BUILD_DIR" ]; then
    echo "❌ Build directory not found. Please run 'npm run build' first."
    exit 1
fi

echo "📦 Build directory found: $BUILD_DIR"

# Create SSH private key file
echo "🔐 Setting up SSH key..."
cat > "$KEY_FILE" << 'EOF'
-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----
EOF

# Set proper permissions for SSH key
chmod 600 "$KEY_FILE"

echo "🔐 Connecting to $SSH_HOST with SSH key..."

# Try rsync with SSH key
echo "📤 Uploading files via rsync with SSH key..."
rsync -avz --delete -e "ssh -i $KEY_FILE -o StrictHostKeyChecking=no" "$BUILD_DIR/" "$SSH_USER@$SSH_HOST:$REMOTE_PATH/"

# Clean up SSH key file
rm -f "$KEY_FILE"

if [ $? -eq 0 ]; then
    echo "✅ Deployment completed successfully!"
    echo "🌐 Your website should now be live at: https://$SSH_HOST"
else
    echo "❌ SSH key deployment failed."
    echo "💡 You may need to manually upload the files using cPanel File Manager:"
    echo "   1. Login to Bluehost cPanel"
    echo "   2. Open File Manager"
    echo "   3. Navigate to /public_html/"
    echo "   4. Upload the contents of the ./dist/ folder"
    exit 1
fi

echo "🎉 Deployment process finished!"