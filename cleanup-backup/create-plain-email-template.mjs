// Script to create a plain email template with the exact text provided
import { writeFileSync } from 'fs';

// The exact text to use in the email template
const emailText = `Hi [Lead's Name],
...
I'm currently available for your wedding on [Wedding Date], and I'd love to learn more about what you're envisioning. What parts of the day are you most excited about? What do you want your photos to remind you of years from now?

Attached, you'll find my wedding collections and pricing to help you get started. You can also take a look at some recent stories I've had the honor of capturing on Instagram: https://www.instagram.com/harielxaviermedia/
...
Instagram: @harielxaviermedia`;

// Create a function that generates a simple email template
const createPlainEmailTemplate = () => {
 ...
 */
export const createPlainThankYouEmailTemplate = (
  name: string,
  email: string,
...
`;
  ...
  writeFileSync('functions/src/plain-email-template.ts', template);
  console.log('Plain email template created at functions/src/plain-email-template.ts');
};

// Run the function
createPlainEmailTemplate();
