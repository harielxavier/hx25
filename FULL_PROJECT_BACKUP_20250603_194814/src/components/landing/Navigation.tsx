          ...
          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              <Link
                key={link.path}
                to={link.path}
                className={`text-sm tracking-wide transition-colors text-black hover:text-gray-600 ${location.pathname === link.path ? 'font-medium' : ''}`}
              >
                {link.label}
              </Link>
            ))}
            ...
            <Link 
              to="/book-now"
              className="px-6 py-2 text-sm tracking-wide transition-colors bg-black text-white hover:bg-gray-900"
            >
              Book Now
            </Link>
            <Link 
              to="/admin"
              className="text-sm tracking-wide transition-colors text-gray-500 hover:text-black"
            >
              Admin
            </Link>
          </div>
