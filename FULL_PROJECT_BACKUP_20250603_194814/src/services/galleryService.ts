...
export interface Gallery {
  ...
  galleryType: 'website' | 'portfolio' | 'client' | string;
  displayLocation?: 'landing' | 'recent' | 'portfolio' | string;
...
export interface GalleryMedia {
  ...
  processingStatus?: 'unprocessed' | 'processing' | 'processed'; // Status of editing
  deliveryStatus?: 'pending' | 'delivered'; // Status of delivery to client
  downloadCount?: number; // Track how many times the image has been downloaded
  viewCount?: number; // Track how many times the image has been viewed
  lastViewed?: Timestamp; // When the image was last viewed
  editHistory?: {
    editor: string;
    timestamp: Timestamp;
    action: string;
...
export const getPublicGalleries = async (): Promise<Gallery[]> => {
  try {
    ...
    // Create the query once to reuse it if needed
    ...
    try {
      ...
      console.log(`Successfully fetched ${galleries.length} public galleries`);
      return galleries;
    } catch (error: any) {
      ...
      // If it's a permission-denied error, we might need to refresh the token
      if (error.code === 'permission-denied') {
        console.warn('Permission denied when fetching galleries, attempting to refresh token...');
        
        // Import the refreshToken function
        ...
        if (newToken) {
          console.log('Token refreshed, retrying gallery fetch...');
          
          // Retry the query with the new token
          const retrySnapshot = await getDocs(galleryQuery);
      ...
      // If it's not a failed-precondition or permission-denied error, rethrow
      throw error;
    }
  } catch (error) {
